...
void forward_signal(int signum) {
    ...
    if (replacement != 0) {
        ...
        DEBUG("Forwarded signal %d (%s) to children.\n", replacement, signum_to_signame(replacement));
    } else {
...
void print_help(char *argv[]) {
    fprintf(stderr,
        ...
        "                        proxying. Signals may be specified as numbers or names\n"
        "                        like USR1 or SIGINT (see -l/--list). To ignore (not\n"
        "                        proxy) a signal, rewrite it to 0. The optional observer\n"
        "                        is a script or executable to execute when signal s is\n"
        "                        received (regardless of any rewriting). It must expect\n"
        "                        no arguments, but the DUMB_INIT_SIGNUM and\n"
        "                        DUMB_INIT_REPLACEMENT_SIGNUM environment variables will\n"
        "                        be set. This option can be specified multiple times.\n"
...
void print_rewrite_signum_help() {
    fprintf(
        stderr,
        "Usage: -r option takes <signum>:<signum>[:<observer>], where "
        "<signum> is between 1 and %d, specified by number or name.\n"
